...
data "coder_parameter" "npm_registry" {
  ...
  description  = "npm registry URL used for global tools and project dependencies."
  type         = "string"
...
resource "coder_agent" "main" {
  ...
  metadata {
    ...
    key          = "toolchain"
    script       = <<-EOT
      bash -lc '. "$HOME/.nvm/nvm.sh" && printf "Node %s | Go %s | Codex %s" "$(node --version)" "$(go env GOVERSION)" "$(codex --version)"'
    EOT
    interval     = 3600
    timeout      = 10
  }
...
resource "docker_image" "omx_codex" {
  name         = "coder-omx-codex:local"
  keep_locally = true
...
resource "docker_container" "workspace" {
  count       = data.coder_workspace.me.start_count
  image       = docker_image.omx_codex.image_id
